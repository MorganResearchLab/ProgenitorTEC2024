...
data.dir <- "/nfs/research1/marioni/mdmorgan/Thymus_Newborn/"
hto_files <- paste0(data.dir, list.files(data.dir, pattern="HTO_counts.tsv"))
id_files <- paste0(data.dir, list.files(data.dir, pattern="HTO_cellids.tsv"))
tag_files <- paste0(data.dir, list.files(data.dir, pattern="HTO_features.tsv"))
...
intest_list <- list()
print(length(hto_matrices))
print(length(hto_ids))
print(length(hto_tags))

for(i in seq_along(1:length(hto_ids))){
  mat <- hto_matrices[[i]]
  #print(length(hto_ids[[i]]))
  #print(length(hto_tags[[i]]))
  #print(dim(mat))
...
for(i in seq_along(1:length(hto_tags))){
  ...
  for(x in seq_along(1:length(hto_tags[[i]]))){
    x.tag <- hto_tags[[i]][x]
    #print(head(intest_list[[i]][x.tag, ]))
    xtag.counts <- intest_list[[i]][x.tag, ]
    print(x.tag)
...
write.table(all.cell.class,
            file=paste0("/nfs/research1/marioni/mdmorgan/Thymus_Newborn/", "HTO_class.tsv"),
