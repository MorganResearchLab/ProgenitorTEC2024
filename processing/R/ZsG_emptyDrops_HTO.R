...
library(biomaRt)
source("/nfs/research/marioni/mdmorgan/Thymus_Newborn/src/single_cell_functions.R")
...
sample_list <- list.files(base_path, pattern="^Newborn", full.names=TRUE)
...
mol_locs <- paste0(sample_list, "/outs/molecule_info.h5")
out_locs <- paste0(sample_list, "/outs/raw_feature_bc_matrix/matrix.mtx.gz")
bc_locs <- paste0(sample_list, "/outs/raw_feature_bc_matrix/barcodes.tsv.gz")
genes_locs <- paste0(sample_list, "/outs/raw_feature_bc_matrix/features.tsv.gz")

message("Reading in unfiltered matrices")
intest_matrices <- list(readMM(out_locs))
intest_bcs <- list(read.table(bc_locs, header=FALSE, stringsAsFactors=FALSE)[, 1])
# extract the genes and features

message("Reading in features")
intest_features <- read.table(genes_locs, sep="\t", header=FALSE, stringsAsFactors=FALSE)
intest_genes <- list(intest_features[grepl(intest_features[, 3], pattern="Gene Expression"), 1])
intest_hto <- list(intest_features[grepl(intest_features[, 3], pattern="Antibody Capture"), 1])
...
message(paste0("Found ", sum(unlist(sig_cells)), " non-empty barcodes"))
...
for(i in seq_along(1:length(cell_barcodes))){
  mat <- intest_cells[[i]]
  rownames(mat) <- intest_features[, 1]
...
for(i in seq_along(1:length(cell_barcodes))){
  ...
  print(sname)
  print(fname)
  message(paste0("Combining counts, cell barcodes and features for: ", fname))
...
message(paste0("Removing ", sum(nonzero.genes <= 1000), " sparse cells"))
## remove cells with excessively high mitochondrial content
biomaRt.connection <- useMart("ensembl", "mmusculus_gene_ensembl", host="www.ensembl.org")
...
writeMM(intest_counts,
        file=paste0("/nfs/research/marioni/mdmorgan/Thymus_Newborn/", args[1], "_counts-HTO.tsv"))
write.table(intest_ids,
            file=paste0("/nfs/research/marioni/mdmorgan/Thymus_Newborn/", args[1], "_counts_colnames-HTO.tsv"),
            sep="\t", row.names=FALSE, quote=FALSE)
write.table(rownames(intest_counts),
            file=paste0("/nfs/research/marioni/mdmorgan/Thymus_Newborn/", args[1], "_counts_rownames-HTO.tsv"),
            sep="\t", row.names=FALSE, quote=FALSE)
