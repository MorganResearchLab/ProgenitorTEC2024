...
goi <- c("Aire", "Psmb11", "Ly6a", "Ccl21a", "Cxcl12", "Krt5", "Krt80", "Avil", 
...
for(x in seq_along(goi)){
    ...
    fr.plot <- ggplot(x.df,
       aes(x=FR1, y=FR2)) +
        # geom_scattermore(data=x.df[, c("FR1", "FR2")], colour='grey80') +
        geom_scattermore(aes(colour=Gene > 1), pointsize=0.75) +
...
specific.genes <- c(specific.genes, gene.df$ensembl_gene_id[gene.df$external_gene_name %in% c("Foxn1", "Aire", "Cd52")])
```
...
specific.genes <- c(specific.genes, gene.df$ensembl_gene_id[gene.df$external_gene_name %in% c("Foxn1", "Psmb11", "Aire", "Cd52")])
```
...
itec.sf.res$NhoodLabel[itec.sf.res$NhoodGroup %in% c(6)] <- "cTEC-Primed Intertypical cTEC (6)"
...
hm.exprs <- nhoodExpression(itec.milo)[unlist(specific.genes), ]
hm.exprs <- hm.exprs[!rownames(hm.exprs) %in% gene.df$ensembl_gene_id[gene.df$external_gene_name %in% c("Aire", "Foxn1", "Psmb11", "Cd52")], ]
row.order <- gene.df[rownames(hm.exprs), ]$external_gene_name
row.order <- row.order[!is.na(row.order)]
rownames(hm.exprs) <- gene.df[rownames(hm.exprs), ]$external_gene_name
