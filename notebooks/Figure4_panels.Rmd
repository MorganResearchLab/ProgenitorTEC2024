...
itec.nhood.annot$NhoodLabel[itec.nhood.annot$NhoodGroup %in% c(6)] <- "cTEC-Primed Intertypical cTEC (6)"
...
ht1 <- netAnalysis_signalingRole_heatmap(itec.chat, plot.paths,  pattern = "outgoing", color.use=samp.cols, width=11, height=14)
ht2 <- netAnalysis_signalingRole_heatmap(itec.chat, plot.paths, pattern = "incoming", color.use=samp.cols, width=12, height=15)
# ht1 + ht2
ht2
```
...
k.mod <- 3
itec.chat <- identifyCommunicationPatterns(itec.chat, slot.name="netP", pattern = "outgoing", k = k.mod, heatmap.show=TRUE, height=21)
```
...
png("~/Dropbox/AgeingExperiment/Perinatal_manuscript/figures/CellChat-Incoming_NMF-heatmap.png",
    height=9, width=6, res=300, units="in")
ht.plot <- Heatmap(hm.vals[col.order, ], col=hm.cols,
        use_raster=TRUE, cluster_rows=TRUE,
        heatmap_legend_param=list(title="Contribution", direction="horizontal"),
        show_column_names=TRUE, cluster_columns=FALSE,
        show_row_names=TRUE, width=ncol(hm.vals)*unit(5, "mm"), 
        left_annotation = col.annot)
draw(ht.plot,  heatmap_legend_side="top")
dev.off()


ht <- Heatmap(hm.vals[col.order,], col=hm.cols,
              use_raster=TRUE, cluster_rows=TRUE,
              heatmap_legend_param=list(title="Contribution", direction="horizontal"),
              show_column_names=TRUE, cluster_columns=FALSE,
              show_row_names=TRUE, width=ncol(hm.vals)*unit(5, "mm"), 
...
ht.plot <- Heatmap(hm.vals[rownames(path.annot.df), ], col=hm.cols,
                   use_raster=TRUE, cluster_rows=TRUE,
...
ht <- Heatmap(hm.vals[rownames(path.annot.df), ], col=hm.cols,
              use_raster=TRUE, cluster_rows=TRUE,
              row_split=path.annot.df$Annotation,
              heatmap_legend_param=list(title="Contribution", direction="horizontal"),
              show_column_names=TRUE, cluster_columns=FALSE, width=ncol(hm.vals)*unit(7, "mm"), 
...
w.norm <- apply(itec.incoming.path[, "Pattern 3", drop=FALSE], 2, FUN=function(RX) RX/max(RX))
w.norm[w.norm < 0.1] <- 0
h.norm <- apply(itec.incoming.cells[, "Pattern 3", drop=FALSE], 2, FUN=function(CX) CX/max(CX))
h.norm[h.norm < 0.1] <- 0
...
ggsave("~/Dropbox/AgeingExperiment/Perinatal_manuscript/figures/CellChat_Ly6d-PathwayContribution-bubble.png",
